...
- Keep local validation runnable even before project-specific build logic exists.

## CI/CD And Release Posture

- Do not keep CI/CD scaffolding that the project does not actively use.
- Add CI only when it can run real repository validation.
- Add release automation only when there is a real artifact, provenance need, and deployment target.
